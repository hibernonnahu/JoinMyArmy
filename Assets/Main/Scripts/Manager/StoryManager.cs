...
public class StoryManager : MonoBehaviour
{
    ...
    private void CallExpresion()//1-id 2-itemname
    {
        Vector3 pos;
        int id = int.Parse(current[1]);
        if (id == -1)
        {
            pos = FindObjectOfType<CharacterMain>().transform.position + Vector3.up * 4;
        }
        else
        {
            pos = GetCharacter(id).transform.position + Vector3.up * 4;
        }
    ...
    {
        FindObjectOfType<CharacterMain>().SetAnimation(current[1]);
        CallStory();
    }
    private void Animation()//1-id 2-animation
    {
        CharacterStory character = GetCharacter(int.Parse(current[1]));
        character.characterEnemy.SetAnimation(current[2]);
        CallStory();
    }
